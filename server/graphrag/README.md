...
GraphRAG (Graph Retrieval-Augmented Generation) 是 Kg4se 项目的核心知识图谱构建模块,实现了从原始文档到结构化知识图谱的完整流程。
...
| 阶段 | 名称 | 核心功能 | 关键技术 | 状态 |
|------|------|----------|----------|------|
| Stage 0 | 语义分块 | 将长文档切分为语义连贯的文本块 | 滑动窗口算法、语义边界检测 | 可选 |
| Stage 1 | 指代消解 | 解析代词和引用，明确实体指向 | NLP指代消解模型 | 默认启用 |
| Stage 2 | 实体链接 | 识别文本中的概念实体并链接到知识库 | 多路召回+精排+NIL检测 | 默认启用 |
| Stage 3 | 论断抽取 | 从文本中提取结构化知识三元组 | LLM提示词工程 | 默认启用 |
| Stage 4 | 谓词治理 | 规范化关系类型，合并同义谓词 | 谓词映射规则 | 默认启用 |
| Stage 5 | 图谱服务 | 将结构化知识持久化到Neo4j | MERGE幂等落库 | 默认启用 |
| Stage 6 | 主题构建 | 将相关概念聚类形成主题社区 | Louvain社区检测算法 | 默认启用 |
| Stage 7 | 指标评估 | 计算知识图谱质量指标 | 孤立节点检测、度分布分析 | 默认启用 |
| Stage 8 | 查询服务 | 实现基于图谱的智能检索 | GraphRAG查询策略 | 可选 |
