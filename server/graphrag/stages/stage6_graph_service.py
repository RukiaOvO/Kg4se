...
# 允许的实体类型（包含传统模式和 GraphRAG Pipeline 模式）
...
# 允许的关系类型（包含传统模式和 GraphRAG Pipeline 模式）
...
class GraphService:
    ...
    def store_relationship(
        ...
        # 2. 存储关系 - 使用 OPTIONAL MATCH 避免创建无标签节点，节点不存在时跳过
        try:
            query = f"""
